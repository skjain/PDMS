...
Begin VB.Form frmGRWastagePacking 
   AutoRedraw      =   -1  'True
   BorderStyle     =   1  'Fixed Single
   Caption         =   "GR to Wastage Packing Module"
   ClientHeight    =   5055
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10335
   ...
   Begin FramePlusCtl.FramePlus FramePlus1 
      ...
      Begin WelchButton.lvButtons_H cmdAdd 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":0000
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdEdit 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":039A
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdDelete 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":0734
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdSave 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":0ACE
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdCancel 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":1858
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdExit 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmGRWastagePacking.frx":1CAA
         cBack           =   -2147483633
      End
   ...
   End
End
Attribute VB_Name = "frmGRWastagePacking"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim FICD As String, BICD As String
...
Private Sub cmdAdd_Click()
    Call btn_sts(False)
    TXTVBDT = Now
    If txtBItem.Enabled Then txtBItem.SetFocus
...
Private Sub cmdEdit_Click()
    SAVEFLAG = False
    frmGRWastagePackingList.DIVCODE = DIVCODE
    frmGRWastagePackingList.M_DBCD = M_DBCD
    frmGRWastagePackingList.BOX_PKG_REQ = BOX_PKG_REQ
    frmGRWastagePackingList.PKGSTCOD = PKG_SCOD
    CHALLAN = Empty
    frmGRWastagePackingList.Show 1
...
On Error GoTo errPRIMARYKEY
    ...
    Dim SQL As String, LOCCOD As String
   ...
   If TEMPRS.State = 1 Then TEMPRS.Close
   TEMPRS.Open "SELECT CODE FROM FINITMMST WHERE COMP='" & compPth & "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & _
   "' AND NAME ='" & txtBItem & "'", CN, adOpenDynamic, adLockOptimistic
   If Not TEMPRS.EOF Then
      BICD = Trim(TEMPRS!CODE)
   End If
   TEMPRS.Close
   ...
   If SAVEFLAG = True Then
     ...
     If BOX_PKG_REQ = "Y" Then
      ...
      SQL = SQL & "NTWGT,RMRK,RECSTAT,BICOD) VALUES('" & compPth & "','" & UNCD & "','" & DIVCODE & _
      "','" & M_DBCD & "','PPF','" & LBLSLIP & "','" & Format(TXTVBDT, "MM/DD/YYYY") & "','" & LBLSLIP & _
      "','" & PKG_SCOD & "','WASTE','" & LOCCOD & "','GRPACK','N','WASTE','" & FICD & "','" & GRADE & "','0','" & MCCD & _
      "','0','0','0','" & Val(TXTQNTY) & "','0','" & Val(TXTQNTY) & "','" & Trim(TXTRMRK) & "','A','" & BICD & "')"
     Else
      SQL = "INSERT INTO PKGMAN(COMP,UNIT,DVCD,DBCD,VTYP,SLIPNO,DATE,PKG_STCOD,PCOD,BOX_COD,COPS_COD,LOTNO,"
      SQL = SQL & "FINITMCOD,GRAD,SUBGRAD,LOCCOD,MCCD,NOB,CPB,GWPB,TWPB,NWPB,QNTY,SYSR,OPER,[USER],REMARKS,RECSTAT,BICOD) VALUES('" & compPth & _
      "','" & UNCD & "','" & DIVCODE & "','" & M_DBCD & "','PPF','" & LBLSLIP & "','" & Format(TXTVBDT, "MM/DD/YYYY") & _
      "','" & PKG_SCOD & "','GRPACK','WASTE','WASTE','WASTE','" & FICD & _
      "','" & GRADE & "','0','" & LOCCOD & "','" & MCCD & "','0','0','0','0','0','" & Val(TXTQNTY) & "','N','+','" & cUName & "','" & TXTRMRK & "','A'," & BICD & "')"
     End If
   ...
   Else
    ...
    If BOX_PKG_REQ = "Y" Then
        ...
        SQL = "UPDATE BOXREGISTER SET LOTNO='WASTE',MCCD='" & MCCD & "',LOCCOD='" & LOCCOD & "',ICOD='" & FICD & "',BICOD='" & BICD & _
    ...
    Else
        ...
        SQL = "UPDATE PKGMAN SET LOTNO='WASTE',MCCD='" & MCCD & "',LOCCOD='" & LOCCOD & "',FINITMCOD='" & FICD & "',BICOD='" & BICD & _
...
Private Sub txtBItem_GotFocus()
    txtBItem.BackColor = RGB(BRED, BGREEN, BBLUE)
End Sub

Private Sub txtBItem_KeyDown(KeyCode As Integer, Shift As Integer)
Me.KeyPreview = False
   If KeyCode = vbKeyF2 Or Trim(txtBItem.Text) = Empty Then
        NEW_VISIBLE = False:  M_DESC = Empty:   Key = Empty
        txtBItem.Text = SearchList1("SELECT  TOP 20 CODE,NAME FROM FINITMMST  WHERE COMP='" & compPth & _
        "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & "'", 0, txtBItem, "List of Base Item Name")
...
Private Sub txtBItem_LostFocus()
    txtBItem.BackColor = vbWhite
End Sub
...
TEMPRS.Open "SELECT NTWGT FROM BOXREGISTER " & _
            "WHERE COMP='" & compPth & "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & _
            "' AND VBNO='" & LBLSLIP & "' ", CN, adOpenDynamic, adLockOptimistic
If Not TEMPRS.EOF Then
    ...
    FIFORS.Open "SELECT NETWGT AS NTWGT,FRESH,WASTAGE FROM GRPACKING " & _
                "WHERE COMP='" & compPth & "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & _
                "' AND ICOD='" & BICD & "' AND WASTAGE > 0 AND RECSTAT<>'D' ORDER BY VBDT,VBNO DESC", CN, adOpenDynamic, adLockOptimistic
            
    Do While Not FIFORS.EOF
   
        TMPQTY = Val(FIFORS!WASTAGE)
        ...
        ElseIf BALQNTY > 0 Or BALQNTY = TMPQTY Then
           FIFORS!WASTAGE = Val(FIFORS!WASTAGE) - BALQNTY
           FIFORS.Update
           BALQNTY = 0
        End If
...
CHKRS.Open "SELECT ISNULL(SUM(NETWGT-FRESH-WASTAGE),0) AS BALWGT FROM GRPACKING WHERE COMP='" & compPth & _
           "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & "' AND ICOD='" & BICD & _
...
Else
   ...
   If Val(CHKRS!BALWGT) < Val(TXTQNTY) Then
   ...
   Dim DSPQTY As Double: DSPQTY = Val(TXTQNTY)
   ...
   RSFIRST.Open "SELECT * FROM GRPACKING WHERE COMP='" & compPth & "' AND UNIT='" & UNCD & "' AND DVCD='" & DIVCODE & _
                "' AND ICOD='" & BICD & "' AND ((NETWGT-FRESH-WASTAGE) > 0)  AND RECSTAT<>'D' ORDER BY VBDT,VBNO", CN, adOpenDynamic, adLockOptimistic
   Do While Not RSFIRST.EOF
      
      TMPQTY = Val(RSFIRST!NETWGT) - Val(RSFIRST!FRESH) - Val(RSFIRST!WASTAGE)
        
        If DSPQTY >= TMPQTY Then
           RSFIRST!WASTAGE = RSFIRST!WASTAGE + TMPQTY
           DSPQTY = DSPQTY - TMPQTY
        ElseIf DSPQTY > 0 Then
           RSFIRST!WASTAGE = RSFIRST!WASTAGE + DSPQTY
           DSPQTY = 0
        End If
