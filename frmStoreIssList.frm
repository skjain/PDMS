...
Begin VB.Form frmStoreIssList 
   ...
   End
End
Attribute VB_Name = "frmStoreIssList"
...
Private Sub CMDGO_Click()
  ...
  Dim SQL As String
  SQL = Empty
  SQL = "SELECT STORETRAN.*,MACMST.NAME FROM STORETRAN INNER JOIN MACMST ON (STORETRAN.PCOD=MACMST.CODE AND STORETRAN.UNIT=MACMST.UNIT AND STORETRAN.DVCD=MACMST.DVCD AND STORETRAN.COMP=MACMST.COMP) WHERE STORETRAN.COMP='" & compPth & "' AND STORETRAN.VTYP='ISS' AND STORETRAN.DVCD='" & DIVISION & "' AND STORETRAN.DATE>='" & Format(txtFrDate.Value, "MM/DD/YYYY") & "' AND STORETRAN.DATE<='" & Format(txtToDate.Value, "MM/DD/YYYY") & "'  AND STORETRAN.RECSTAT<>'D'  AND STORETRAN.UNIT='" & UNCD & "' ORDER BY STORETRAN.DATE,STORETRAN.VBNO"
...
Private Sub cmdOK_Click()
  ...
  Dim SQL As String
  SQL = "SELECT * FROM STORETRAN WHERE COMP='" & compPth & "' AND VTYP='ISS' AND SRNO='" & SEL_SRNO & "' AND RECSTAT<>'D'  AND STORETRAN.UNIT='" & UNCD & "' AND STORETRAN.DVCD='" & DIVISION & "'"
  If EDTDAT.State = 1 Then EDTDAT.Close
  EDTDAT.Open SQL, CN, adOpenDynamic, adLockOptimistic
  If EDTDAT.EOF Then
     lstBill.SetFocus
