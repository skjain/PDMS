...
Begin VB.Form frmRPT_GRWastePackingRegister 
   AutoRedraw      =   -1  'True
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "GR to Wastage Packing Register"
   ClientHeight    =   5280
   ClientLeft      =   1680
   ClientTop       =   1170
   ClientWidth     =   6510
   ...
   End
End
Attribute VB_Name = "frmRPT_GRWastePackingRegister"
...
On Error GoTo errPreview
    ...
    rptsql = rptsql & " AND ({BOXREGISTER.VTYP}='PPF' OR {BOXREGISTER.VTYP}='OPN' OR {BOXREGISTER.VTYP}='WST') AND {BOXREGISTER.UNIT}='" & txtUNIT.Tag & "' AND {BOXREGISTER.RECSTAT}<>'D' AND {BOXREGISTER.DVCD}='" & txtDVCD.Tag & "'"
    rptsql = rptsql & " AND {BOXREGISTER.PCOD}='GRPACK' AND {BOXREGISTER.LOTNO}='WASTE' "
    ...
    If Not txtLoc = Empty Then rptsql = rptsql & " AND {BOXREGISTER.LOCCOD} = '" & txtLoc.Tag & "'"
     ...
     If cmbFormat.ListIndex = 0 Then
        ReportName = App.PATH & "\Reports\DateWiseGRWastePackingRegister.rpt"
        RPTN = "GR TO WASTAGE PACKING REGISTER"
     End If
...
Private Sub FillReportFormat()
  cmbFormat.Clear
  cmbFormat.AddItem "GR to Wastage Packing Register"
