...
Begin VB.Form frmRPT_InhouseGRNReg 
   AutoRedraw      =   -1  'True
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Inhouse GRN Register"
   ClientHeight    =   5355
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   6825
   ...
   Begin VB.Frame Frame4 
      ...
      Begin WelchButton.lvButtons_H cmdpreview 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_InhouseGRNReg.frx":0000
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdCancel 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_InhouseGRNReg.frx":0452
         cBack           =   -2147483633
      End
   ...
   End
End
Attribute VB_Name = "frmRPT_InhouseGRNReg"
...
Me.KeyPreview = False
    ...
    If cboReports.ListIndex = 0 Then
       ReportName = App.PATH & "\Reports\Inhouse_JobWork_DateWiseReg.rpt"
       RPTN = "DATEWISE GRN REGISTER FOR INHOUSE JOBWORK"
    ElseIf cboReports.ListIndex = 1 Then
       ReportName = App.PATH & "\Reports\Inhouse_JobWork_PartyWiseReg.rpt"
       RPTN = "PARTYWISE GRN REGISTER FOR INHOUSE JOBWORK"
    End If
    
    rptsql = Empty
    rptsql = "{JOBIN.COMP}='" & compPth & "' AND {JOBIN.UNIT} = '" & txtUNIT.Tag & "' "
    rptsql = rptsql & "AND {JOBIN.DATE}>=DATE(" & Year(dtFrom) & "," & Month(dtFrom) & "," & Day(dtFrom) & _
    ") AND {JOBIN.DATE}<=DATE(" & Year(dtTo) & "," & Month(dtTo) & "," & Day(dtTo) & ") AND {JOBIN.RECSTAT}<>'D' "
    
    rptsql = rptsql & "AND {JOBIN.VTYP}='IVR' AND {JOBIN.DBCD}='000002'"
...
Private Sub Form_Activate()
 ...
 With cboReports
       .AddItem "Inhouse GRN Register"
       .AddItem "Partywise Inhouse GRN Register"
