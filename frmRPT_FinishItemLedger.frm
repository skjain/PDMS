...
Begin VB.Form frmRPT_FinishItemLedger 
   AutoRedraw      =   -1  'True
   ...
   Begin VB.Frame Frame4 
      ...
      Begin WelchButton.lvButtons_H cmdpreview 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_FinishItemLedger.frx":0000
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdCancel 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_FinishItemLedger.frx":0452
         cBack           =   -2147483633
      End
   ...
   End
End
Attribute VB_Name = "frmRPT_FinishItemLedger"
...
On Error GoTo errPreview
    ...
    If cboReports.ListIndex = 0 Then
       ReportName = App.PATH & "\Reports\FinishItemLedger.rpt"
       RPTN = "Finish Item Ledger"
    End If
    
    rptsql = "{VW_FINITEM_LEDGER.COMP}='" & compPth & "' AND {VW_FINITEM_LEDGER.UNIT} = '" & txtUNIT.Tag & "' AND {VW_FINITEM_LEDGER.DVCD}='" & txtDVCD.Tag & "' "
    rptsql = rptsql & " AND {VW_FINITEM_LEDGER.DATE}<=DATE(" & Year(dtTo) & "," & Month(dtTo) & "," & Day(dtTo) & ") "
  
    If txtITEM <> Empty Then rptsql = rptsql & " AND {VW_FINITEM_LEDGER.ITEM}='" & txtITEM & "'"
...
Private Sub txtDVCD_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF2 Or (txtDVCD = Empty And KeyCode = 13) Then
        NEW_VISIBLE = False
        M_DESC = Empty
        Key = Empty
        txtDVCD.Text = SearchList1("SELECT  TOP 20 CODE, NAME FROM DIVMST WHERE COMP='" & compPth & "' AND UNIT='" & txtUNIT.Tag & "' AND CODE<>'000001' AND RECSTAT='A'", 0, "", "List Of Division")
        txtDVCD.Tag = Key
...
If txtUNIT = Empty Then txtUNIT.SetFocus
If txtDVCD = Empty Then txtDVCD.SetFocus
    ...
    If KeyCode = vbKeyF2 Then
        ...
        txtITEM = SearchList1("Select TOP 20 Code,Name From FINITMMST WHERE COMP='" & compPth & _
                              "' AND UNIT='" & txtUNIT.Tag & "' AND DVCD='" & txtDVCD.Tag & "' AND CODE<>'000001'", 0, Empty, "Select Finish Item")
        txtITEM.Tag = Key
    ElseIf KeyCode = vbKeyDelete Or KeyCode = vbKeyBack Then
        txtITEM = Empty
        txtITEM.Tag = Empty
...
Private Sub SetSQL()
 ...
 End If
    ...
    If txtDVCD <> Empty And UCase(txtITEM) <> "RCOPS" Then rptsql = rptsql & " AND {IGMMST.NAME}='" & txtDVCD & "'"
