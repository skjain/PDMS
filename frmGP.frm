...
Dim GPNO As String
    ...
    ReportName = App.PATH & "\Reports\FINISH GATE PASS.rpt"
    ...
    'CHALLAN ARRAY
     ...
     Next
      ...
      rptsql = "{SPTRAN.COMP}='" & compPth & "' AND {SPTRAN.UNIT}='" & txtUNIT.Tag & _
               "'  And {SPTRAN.VTYP}='DPF' AND {SPTRAN.RECSTAT} <>'D' " & _
               "AND {SPTRAN.GATEPASSNO} IN [" & M_GPN & "]"
...
SQL = "SELECT GPMST.GPDT AS DATE,GPMST.GPNO AS VBNO,TRANSPORTMST.NAME,VHCLMST.NAME AS VEHICLE," & _
"GPMST.GROSSWGT AS TQTY FROM GPMST " & _
"INNER JOIN TRANSPORTMST ON GPMST.TRCD=TRANSPORTMST.CODE " & _
"INNER JOIN VHCLMST ON GPMST.VHCD=VHCLMST.CODE " & _
"WHERE GPMST.COMP='" & compPth & "' AND UNIT='" & UNCD & _
"' AND GPMST.GPDT >='" & Format(dtOPDT.Value, "mm/dd/yyyy") & _
"' AND GPMST.GPDT <='" & Format(dtENDT.Value, "mm/dd/yyyy") & "' AND RECSET<>'D' GROUP BY GPMST.GPDT," & _
"GPMST.GPNO,TRANSPORTMST.NAME,VHCLMST.NAME,GPMST.GROSSWGT ORDER BY GPMST.GPNO,GPMST.GPDT "
    ...
    Do While Not rsTemp.EOF
        ...
        Item.SubItems(2) = Trim(rsTemp!NAME & "")
        Item.SubItems(3) = Trim(rsTemp!VEHICLE & "")
        Item.SubItems(4) = Format(rsTemp!TQTY, "#.000")
...
Private Sub txtDVCD_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyF2 Or (KeyCode = vbKeyReturn And txtDVCD = Empty) Then
        ...
        M_DESC = Empty
        Key = Empty
        txtDVCD = SearchList1("SELECT TOP 20 Code,NAME From DIVMST Where COMP='" & compPth & "' and Unit='" & txtUNIT.Tag & "' ADN RECSTAT='A'", 0, Empty, "Select Division")
        txtDVCD.Tag = Key
