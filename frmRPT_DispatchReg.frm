...
Begin VB.Form frmRPT_DispatchReg 
   AutoRedraw      =   -1  'True
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Dispatch Register With D.O"
   ClientHeight    =   6915
   ClientLeft      =   2940
   ClientTop       =   870
   ClientWidth     =   6660
   ...
   Begin VB.Frame Frame4 
      ...
      Begin WelchButton.lvButtons_H cmdpreview 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_DispatchReg.frx":0000
         cBack           =   -2147483633
      End
      ...
      Begin WelchButton.lvButtons_H cmdExit 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_DispatchReg.frx":0452
         cBack           =   -2147483633
      End
   ...
   End
End
Attribute VB_Name = "frmRPT_DispatchReg"
...
On Error GoTo errPreview
    ...
    rptsql = "{ORDTRN.COMP}='" & compPth & "' AND {ORDTRN.VTYP}='DPF' And {ORDTRN.SLIPDATE}>=DATE(" & Year(dtFrom) & _
    "," & Month(dtFrom) & "," & Day(dtFrom) & ") AND {ORDTRN.SLIPDATE}<=DATE(" & Year(dtTo) & "," & Month(dtTo) & _
    "," & Day(dtTo) & ") And {ORDTRN.RECSTAT}<>'D' AND {ORDTRN.UNIT}='" & txtUNIT.Tag & "'"
    
    If Not txtDVCD.Text = Empty Then rptsql = rptsql & "  AND {ORDTRN.DVCD}='" & txtDVCD.Tag & "' "
    If Not cmbDispatchType = Empty Then rptsql = rptsql & "  AND {ORDTRN.DBCD}='" & GetDispatchCode & "' "
    If Not TXTBRCD = Empty Then rptsql = rptsql & " AND {ORDTRN.BRCD}='" & TXTBRCD.Tag & "' "
    If Not txtSalesMan = Empty Then rptsql = rptsql & " AND {ORDTRN.RDBC}='" & txtSalesMan.Tag & "' "
    If Not txtParty = Empty Then rptsql = rptsql & " AND {ORDTRN.PCOD}='" & txtParty.Tag & "' "
     If Not txtConsignee = Empty Then rptsql = rptsql & " AND {PADDMST.NAME}='" & txtConsignee & "' "
    
    If Not txtltno = Empty Then rptsql = rptsql & " AND {ORDTRN.ltno}='" & txtltno.Tag & "' "
    If Not TXTITEM = Empty Then rptsql = rptsql & " AND {ORDTRN.icod}='" & TXTITEM.Tag & "' "
    If Not txtgrade = Empty Then rptsql = rptsql & " AND {ORDTRN.GRAD}=" & txtgrade.Tag & " "
    If Not txtsubgrad = Empty Then rptsql = rptsql & " AND {ORDTRN.SUBGRD}='" & txtsubgrad.Tag & "' "
                   
    Select Case cboFormats.ListIndex
       Case 0
           ReportName = App.PATH & "\Reports\Dispatch With DO.rpt"
