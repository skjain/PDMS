...
Begin VB.Form frmStoreIssue 
   ...
   End
End
Attribute VB_Name = "frmStoreIssue"
...
Private Sub SETFLEX()
  ...
  ITMFLEX.TextMatrix(0, 3) = "Issue Qty"
  ...
  For I = 1 To ITMFLEX.Rows - 1
    ...
    'FIFO
      If SAVEFLAG = True And FIFOREQ = "Y" Then
         FIFORATE = FindFIFORate(ITMFLEX.TextMatrix(I, 1), Val(ITMFLEX.TextMatrix(I, 4)))
      End If
    '------------------------
    ...
    SAVDAT!AMNT = Val(SAVDAT!RATE) * Val(ITMFLEX.TextMatrix(I, 3))
...
Private Sub DELETEISS()
  CN.Execute "DELETE FROM STORETRAN WHERE COMP='" & compPth & "' AND UNIT='" & UNCD & _
             "' AND VTYP='ISS' AND DBCD='" & M_DBCD & "' AND VBNO='" & TXTVBNO & "'"
End Sub
