...
Begin VB.Form frmRPT_BookingStatus 
   ...
   Begin VB.Frame Frame4 
      ...
      Begin WelchButton.lvButtons_H cmdpreview 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_BookingStatus.frx":0000
         cBack           =   -2147483633
      End
      Begin WelchButton.lvButtons_H cmdExit 
         ...
         Mode            =   0
         Value           =   0   'False
         Image           =   "frmRPT_BookingStatus.frx":0452
         cBack           =   -2147483633
      End
   ...
   End
End
Attribute VB_Name = "frmRPT_BookingStatus"
...
Dim FLAG As Boolean
FLAG = True
If cboReports.ListIndex = 2 Then
   FLAG = False
End If
  txtBroker = Empty
  dtFrom.Enabled = FLAG
  dtTo.Enabled = FLAG
  txtBroker.Enabled = FLAG
  Label6.Enabled = FLAG
  Label4.Enabled = FLAG
  Label8.Enabled = FLAG
End Sub
...
On Error GoTo errPreview
    ...
    rptsql = "{ORDMAN.COMP}='" & compPth & "' AND {ORDMAN.UNIT} = '" & txtUNIT.Tag & _
    "' AND {ORDMAN.ORDT}>=DATE(" & Year(dtFrom) & "," & Month(dtFrom) & "," & Day(dtFrom) & _
    ") AND {ORDMAN.ORDT}<=DATE(" & Year(dtTo) & "," & Month(dtTo) & "," & Day(dtTo) & ") AND {ORDMAN.FIN_APRV}='O'  AND {ORDMAN.RECSTAT}<>'D' "
      
    If txtDVCD <> Empty Then rptsql = rptsql & " AND {ORDMAN.DCOD}='" & txtDVCD.Tag & "'"
    ...
    If txtParty <> Empty Then rptsql = rptsql & " AND {ORDMAN.PCOD}='" & txtParty.Tag & "'"
    ...
    If cboReports.ListIndex = 0 Then
       ReportName = App.PATH & "\Reports\ITEM+GRADE WISE SUMMARY.RPT"
       RPTN = "ITEM+GRADE WISE SUMMARY "
    ElseIf cboReports.ListIndex = 1 Then
       ReportName = App.PATH & "\Reports\PARTY+ITEM+GRADE WISE SUMMARY.RPT"
       RPTN = "PARTY+ITEM+GRADE WISE SUMMARY "
    ElseIf cboReports.ListIndex = 2 Then
       ReportName = App.PATH & "\Reports\AGENT+ITEM+GRADE WISE SUMMARY.RPT"
       RPTN = "AGENT+ITEM+GRADE WISE SUMMARY "
    ElseIf cboReports.ListIndex = 3 Then
       ReportName = App.PATH & "\Reports\Item+Monthwise OrderAgainstDispatch.RPT"
       RPTN = "Item+Monthwise OrderAgainstDispatch"
    End If
    ...
    If cboReports.ListIndex <> 2 Then .Formulas(4) = "PERIOD='" & PERIOD & "'"
    ...
    RPTN = RPTN + Space(5) + ReportName
        ...
        ElseIf ReadConfigMaster("000031", 8, "R") Then
             CRPT.WindowShowPrintBtn = True
             CRPT.WindowShowPrintSetupBtn = True
        Else
...
Call ColorComponent(Me)
    ...
    With cboReports
        .AddItem "ITEM+GRADE WISE SUMMARY"
        .AddItem "PARTY+ITEM+GRADE WISE SUMMARY"
        .AddItem "AGENT+ITEM+GRADE WISE SUMMARY"
        .AddItem "ITEM+MONTHWISE ORDER AGAINST DISPACTH"
        .ListIndex = 0
    End With
...
Private Sub txtBroker_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF2 Then
        ...
        M_DESC = Empty
        Key = Empty
        txtBroker.Text = SearchList1("SELECT  TOP 20 CODE, NAME FROM REFMST WHERE CATA='B'", 0, "", "List Of Agent")
        txtBroker.Tag = Key
...
Private Sub txtParty_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF2 Then
        ...
        M_DESC = Empty
        Key = Empty
        txtParty.Text = SearchList1("SELECT  TOP 20 CODE, NAME FROM ACCMST ", 0, "", "List Of Party")
        txtParty.Tag = Key
    ElseIf KeyCode = vbKeyDelete Or KeyCode = vbKeyBack Then
        txtParty = Empty
        txtParty.Tag = Empty
