...
Begin VB.Form frmRPT_ReturnableDatewiseSummary 
   AutoRedraw      =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmRPT_ReturnableDatewiseSummary"
...
If PTYP = "PALLET" Then
   ...
   ReportName = App.PATH & "\Reports\DatewiseReturnablePalletSummary.rpt"
ElseIf PTYP = "COPS" Then
   Call SetViewForCops
   ReportName = App.PATH & "\Reports\DatewiseReturnableCopsSummary.rpt"
End If
...
On Error GoTo VWERR
'ISNULL(SUM(DAMT),0)
QRY = "CREATE VIEW VW_RETURNABLE_DATEWISE_SUMMARY AS " & _
   "SELECT PKGSTK.COMP,PKGSTK.UNIT,UNTCFG.CUNT,PKGSTK.DATE,PKGSTK.OPER,ISNULL(SUM(TOPPLY),0) AS PCS,CONVERT(CHAR(20),'TOP') AS PLYNAM FROM PKGSTK " & _
   "INNER JOIN UNTCFG ON UNTCFG.COMP = PKGSTK.COMP AND UNTCFG.UNIT = PKGSTK.UNIT " & _
   ...
   " GROUP BY PKGSTK.COMP,PKGSTK.UNIT,UNTCFG.CUNT,PKGSTK.DATE,PKGSTK.OPER " & _
   "UNION " & _
   "SELECT PKGSTK.COMP,PKGSTK.UNIT,UNTCFG.CUNT,PKGSTK.DATE,PKGSTK.OPER,ISNULL(SUM(BOTTOMPLY),0) AS PCS,CONVERT(CHAR(20),'BOTTOM') AS PLYNAM FROM PKGSTK " & _
   "INNER JOIN UNTCFG ON UNTCFG.COMP = PKGSTK.COMP AND UNTCFG.UNIT = PKGSTK.UNIT " & _
   "Where PKGSTK.COMP='" & compPth & "' AND PKGSTK.UNIT='" & txtUNIT.Tag & _
   "' AND PKGSTK.RECSTAT<>'D' " & _
   "  AND PKGSTK.DATE>='" & Format(dtFrom.Value, "MM/DD/YYYY") & _
   "' AND PKGSTK.DATE<='" & Format(dtTo.Value, "MM/DD/YYYY") & "' AND PKGSTK.BOTTOMPLY > 0 " & _
   " GROUP BY PKGSTK.COMP,PKGSTK.UNIT,UNTCFG.CUNT,PKGSTK.DATE,PKGSTK.OPER "
...
CN.Execute "IF ( OBJECT_ID('VW_RETURNABLE_DATEWISE_SUMMARY') IS NOT NULL ) DROP VIEW VW_RETURNABLE_DATEWISE_SUMMARY "
CN.Execute QRY
...
Private Function AddSQLForPallet(PLYNAM As String) As String
 ...
 "SELECT PKGSTK.COMP,PKGSTK.UNIT,UNTCFG.CUNT,PKGSTK.DATE,PKGSTK.OPER,ISNULL(SUM(" & PLYNAM & "),0) AS PCS,CONVERT(CHAR(20),'" & PLYNAM & "') AS PLYNAM FROM PKGSTK " & _
 "INNER JOIN UNTCFG ON UNTCFG.COMP = PKGSTK.COMP AND UNTCFG.UNIT = PKGSTK.UNIT " & _
 "Where PKGSTK.COMP='" & compPth & "' AND PKGSTK.UNIT='" & txtUNIT.Tag & _
 "' AND PKGSTK.RECSTAT<>'D' " & _
 "  AND PKGSTK.DATE>='" & Format(dtFrom.Value, "MM/DD/YYYY") & _
