...
Begin VB.Form frmGRWastagePackingList 
   Caption         =   "GR to Wastage Packing List"
   ClientHeight    =   6300
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   10845
   ...
   End
End
Attribute VB_Name = "frmGRWastagePackingList"
...
Private Sub cmdCancel_Click()
    frmGRWastagePacking.CHALLAN = Empty
    Unload Me
End Sub
...
Do While Not EDTDAT.EOF
 ...
 Loop
    ...
    cmdOk.Enabled = True
    cmdOk.Default = True
    If frmGRWastagePackingList.Visible = True Then lstBill.SetFocus
End Sub
...
SQL = "SELECT BOXREGISTER.*,FINITMMST.NAME AS ITEM,BASEITMMST.NAME AS BITEM,MACMST.NAME AS MACHINE,LOCMST.NAME AS LOCATION,GRDMST.GRAD AS GRADE FROM "
SQL = SQL & "BOXREGISTER INNER JOIN FINITMMST ON FINITMMST.COMP=BOXREGISTER.COMP AND FINITMMST.UNIT=BOXREGISTER.UNIT AND "
SQL = SQL & "FINITMMST.DVCD=BOXREGISTER.DVCD AND FINITMMST.CODE=BOXREGISTER.ICOD LEFT JOIN LOCMST ON BOXREGISTER.LOCCOD=LOCMST.CODE "
SQL = SQL & "INNER JOIN FINITMMST BASEITMMST ON BASEITMMST.COMP=BOXREGISTER.COMP AND BASEITMMST.UNIT=BOXREGISTER.UNIT AND "
SQL = SQL & "BASEITMMST.DVCD=BOXREGISTER.DVCD AND BASEITMMST.CODE=BOXREGISTER.BICOD "
...
"' AND VBNO = '" & CHLNNO & "' "
    ...
    With frmGRWastagePacking
        .CHALLAN = CHLNNO
        .LBLSLIP.Caption = CHLNNO
        .txtITEM.Text = EDTDAT!Item & ""
        .txtBItem.Text = EDTDAT!BItem & ""
...
SQL = "SELECT PKGMAN.*,FINITMMST.NAME AS ITEM,BASEITMMST.NAME AS BITEM,MACMST.NAME AS MACHINE,LOCMST.NAME AS LOCATION FROM "
SQL = SQL & "PKGMAN INNER JOIN FINITMMST ON FINITMMST.COMP=PKGMAN.COMP AND FINITMMST.UNIT=PKGMAN.UNIT AND "
SQL = SQL & "FINITMMST.DVCD=PKGMAN.DVCD AND FINITMMST.CODE=PKGMAN.FINITMCOD LEFT JOIN LOCMST ON PKGMAN.LOCCOD=LOCMST.CODE "
SQL = SQL & "INNER JOIN FINITMMST BASEITMMST ON BASEITMMST.COMP=PKGMAN.COMP AND BASEITMMST.UNIT=PKGMAN.UNIT AND "
SQL = SQL & "BASEITMMST.DVCD=PKGMAN.DVCD AND BASEITMMST.CODE=PKGMAN.BICOD "
SQL = SQL & "LEFT JOIN MACMST ON MACMST.CODE=PKGMAN.MCCD WHERE PKGMAN.COMP='" & compPth & "' AND PKGMAN.UNIT='" & UNCD & _
